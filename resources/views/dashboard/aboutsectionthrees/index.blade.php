...
@section('content')
    ...
    <h1 class="page-title">السكشن الثالث</h1>
    <a href="{{route('dashboard.aboutsectionthree.create')}}" class="btn green"> أضافة
                        ...
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            @foreach($aboutsectionthrees as $index=>$aboutsectionthree)
                                <tr>
                                    ...
                                    <td>{{$aboutsectionthree->title}}</td>
                                    <td>{{$aboutsectionthree->desc}}</td>
                                    
                                    <td>
                                        <a href="{{route('dashboard.aboutsectionthree.edit', $aboutsectionthree->id)}}" class="btn btn-outline btn-circle btn-sm purple">
                                            <i class="fa fa-edit"></i> تعديل </a>
                                        <form method="POST" onclick="return confirm('Are you sure?')" action="{{route('dashboard.aboutsectionthree.destroy', $aboutsectionthree->id)}}"  style="display:inline" >
                                            {{ csrf_field() }}
                                            {{ method_field('delete') }}
