...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span> اعدادات صفحة الكاترة </span>
            </li>
        </ul>

    </div>
    <h1 class="page-title"> اعدادات صفحة الدكاترة </h1>
    <a href="{{route('dashboard.doctor.create')}}" class="btn green"> أضافة
                        ...
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            @foreach($doctors as $index=>$doctor)
                                <tr>
                                    ...
                                    <td>{{$doctor->facebook_link}}</td>
                                    <td>{{$doctor->twitter_link}}</td>
                                    <td>{{$doctor->linkedin_link}}</td>
                                    <td>{{$doctor->youtube_link}}</td>
                                    

                                    <td>
                                        <img src="{{asset('uploads/doctors/'.$doctor->img)}}"  alt="Image" style="width:50px;height:50px;">
                                    </td>


                                    <td>
                                        <a href="{{route('dashboard.doctor.edit', $doctor->id)}}" class="btn btn-outline btn-circle btn-sm purple">
                                            <i class="fa fa-edit"></i> تعديل </a>
                                        <form method="POST" onclick="return confirm('Are you sure?')" action="{{route('dashboard.doctor.destroy', $doctor->id)}}"  style="display:inline" >
                                            {{ csrf_field() }}
                                            {{ method_field('delete') }}
