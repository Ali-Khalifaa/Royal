...
@section('content')
    ...
    <h1 class="page-title">السكشن الأول</h1>
    {{-- <a href="{{route('dashboard.servicesone.create')}}" class="btn green"> أضافة
                        ...
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            @foreach($servicesones as $index=>$servicesone)
                                <tr>
                                    ...
                                    <td>{{$servicesone->title}}</td>
                                    <td>{{$servicesone->desc}}</td>
                                    
                                    <td>
                                        <a href="{{route('dashboard.servicesone.edit', $servicesone->id)}}" class="btn btn-outline btn-circle btn-sm purple">
                                            <i class="fa fa-edit"></i> تعديل </a>
                                        {{-- <form method="POST" onclick="return confirm('Are you sure?')" action="{{route('dashboard.servicesone.destroy', $servicesone->id)}}"  style="display:inline" >
                                            {{ csrf_field() }}
                                            {{ method_field('delete') }}
