...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span> رسائل الصفحة تواصل معنا </span>
            </li>
        </ul>
                ...
                <div class="portlet-body">
                    <div class="table-scrollable">
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            <tbody>
                                @foreach($contactComments as $index=>$contactComment)
                                    <tr>
                                        <td scope="row">{{ $index + 1}}</td>
    
                                        <td>{{$contactComment->name }}</td>
                                        <td>{{$contactComment->subject }}</td>
                                        <td>{{$contactComment->phone }}</td>
                                        <td>{{$contactComment->email }}</td>
                                        <td>{{$contactComment->messege }}</td>
